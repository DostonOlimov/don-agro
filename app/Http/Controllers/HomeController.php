...
class HomeController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $all_cer_count = Sertificate::count();
           ...
           $crops = CropsName::select('crops_name.name','crops_name.id','crops_name.measure_type',
        ...
        $app_states = $app_states->where('applications.app_number','!=',0)
            ->whereYear('applications.date',$year)
            ->groupBy('tbl_states.id', 'tbl_states.name')
            ->orderBy('application_count', 'desc')
            ->get();
        $crops = $crops->where('applications.app_number','!=',0)
            ->whereYear('applications.date',$year)
            ->groupBy("crops_name.name",'crops_name.id','crops_name.measure_type')
            ->orderBy('count','desc')
            ->get();
