...
<body>
	@if(!empty($driverLicence))
			<div class="front-side" style="position: relative; width: 86mm; height: 54mm;background-size: cover; background-image: url({{ URL::asset('resources/views/layouts/assets/images/dr-licence1.png') }})" >
				...
				<p style="top: 153px;  left: 15px;">7. _____________</p>
				...
				<p style="top: 86px;  left: 126px; max-width: 160px; min-height: 24px;"> {{' '.date('d.m.Y',strtotime($driverLicence->given_date))}}</p>
				<p style="top: 86px;  left: 209px; min-height: 32px"> {{' '.date('d.m.Y',strtotime('+10 years',strtotime($driverLicence->given_date))) }}</p>

				<p style="top: 98px;  left: 126px;"> {{ getCityName($driverLicence->given_city_id) }}</p>
				@if(file_exists($_SERVER['DOCUMENT_ROOT'].'/public/uploads/driver-'.$driverLicence->owner_id.'.jpeg'))
					<img src="{{ URL::asset('public/uploads/driver-'.$driverLicence->owner_id.'.jpeg') }}?f={{rand(1,10)}}" style="top: 53.1px; left: 14.7px; width: 20.15mm; height: 25.15mm; position: absolute;">
				@else
					<img src="{{ URL::asset('public/uploads/driver-64.jpeg') }}" style="top: 53.1px; left: 14.7px; width: 20.15mm; height: 25.15mm; position: absolute;">
				@endif
				...
				<p style="top: 131px;  left: 126px; width: 140px;">{{' '.$driverLicence->city.', '.$driverLicence->address}}</p>
			...
			<div class="back-side" style="">
				...
				<p style="top: 135px;  left: 196px;">{{$typesArray[5]?date('d.m.Y',strtotime($typesArray[5]['given_date'])):''}}</p>
