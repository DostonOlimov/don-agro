...
<style>
	...
	@if (CheckAccessUser('driver_lic', 'read'))
	    <div class="section">
			<!-- PAGE-HEADER -->
								...
								<table id="examples1" class="table table-striped table-bordered nowrap" style="margin-top:20px; width:100%;">
									...
									<thead>
										...
										<tr>
											...
											<th class="border-bottom-0 border-top-0">Guvohnoma</th>
											<th class="border-bottom-0 border-top-0">Toifa</th>
											<th class="border-bottom-0 border-top-0">Haydovchi</th>
											<th class="border-bottom-0 border-top-0">{{ trans('app.SHIR/STIR') }}</th>
											<th class="border-bottom-0 border-top-0">Viloyat / Tuman</th>
									...
									<?php $i=1;?>
										...
										@if(!empty($driverLicences))
											...
											@foreach($driverLicences as $licence)
												...
												<tr>
													<td>{{ (request()->input('page', 1) - 1) * 50 + $i }}</td>
													<td>
														@if (CheckAccessUser('driver_lic', 'read'))
													...
													<td>{{$givenTypes}}</td>
													<td>
														@if (CheckAccessUser('customer_add', 'read'))
													...
													<td>{{ $licence->pinfl ?: ($licence->id_number ?: $licence->inn) }}</td>
                                                    <td>
                                                        @php
                                                            $area = optional($areas->firstWhere('id', $licence->city_id));
                                                        @endphp

                                                        {{optional(optional($area)->region)->name}}, {{optional($area)->name}}
                                                    </td>
