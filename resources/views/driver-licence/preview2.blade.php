...
@if(!empty($driverLicence))
    ...
    <div class="front-side"
         style="position: relative; width: 86mm; height: 54mm;background-size: cover; background-image: url({{ URL::asset('resources/views/layouts/assets/images/dr-licence1.png') }})">
        ...
        <p style="top: 75px;  left: 126px;"><span
                style="text-transform: uppercase;">{{getCityName($driverLicence->customer->birthplace_id)}}</span>{{ ', '.date('d.m.Y',strtotime($driverLicence->d_o_birth))}}
        </p>
        ...
        <p style="top: 98px;  left: 126px;"
           class="text-capitalize"> {{ getCityName($driverLicence->given_city_id) }}</p>

        @if(file_exists($_SERVER['DOCUMENT_ROOT'].'/public/uploads/drivers/'.date('Y/m/d', $givenDate).'/driver-'.$driverLicence->owner_id.'.jpeg'))
            <img
                src="{{ URL::asset('public/uploads/drivers/'.date('Y/m/d', $givenDate).'/driver-'.$driverLicence->owner_id.'.jpeg') }}?f={{rand(1,10)}}"
                style="top: 53.1px; left: 14.7px; width: 20.15mm; height: 25.15mm; position: absolute;">
        @else
            <img src="{{ URL::asset('public/uploads/drivers/driver-default.jpeg') }}"
                 style="top: 53.1px; left: 14.7px; width: 20.15mm; height: 25.15mm; position: absolute;">
        @endif
        ...
        @if(file_exists($_SERVER['DOCUMENT_ROOT'].'/public/uploads/signatures/'.date('Y/m/d', $givenDate).'/signature-driver-'.$driverLicence->owner_id.'.png'))
            ...
            <img class="signature"
                 src="{{ URL::asset('public/uploads/signatures/'.date('Y/m/d', $givenDate).'/signature-driver-'.$driverLicence->owner_id.'.png?v='.strtotime('now')) }}">
        ...
        <p style="top: 109px;  left: 126px;"> {{$driverLicence->pinfl?:(($driverLicence->id_number ?: $driverLicence->inn) ?: 'XXX')}}</p>
        ...
        <p style="top: 131px;  left: 126px; width: 140px;"
           class="text-capitalize">{{' '.$driverLicence->city.', '.$driverLicence->address}}</p>


        <?php

        $types = $driverLicence->type;
    ...
    <div class="back-side" style="">
        ...
        <p style="top: 135px;  left: 196px;"
           class="text-capitalize">{{$typesArray[5]?date('d.m.Y',strtotime($typesArray[5]['given_date'])):''}}</p>

        <p style="top: 135px;  left: 235px;">{{$typesArray[5]?date('d.m.Y',strtotime('+10 years',strtotime($typesArray[5]['given_date']))):''}}</p>
        <p style="top: 160px;  left: 210px;">
