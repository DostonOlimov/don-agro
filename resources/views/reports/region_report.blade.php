...
@section('styles')
    <style>
        ...
        .input-container input[type="number"] {
            padding-right: 30px;
            /* Adjust this value as needed to make space for the icon */
        }
    ...
    @can('viewAny', \App\Models\Application::class)
        @can('add_number', \App\Models\LaboratoryResult::class)
            <div class="section">
                ...
                <div class="page-header">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            ...
                            {{ trans('message."Qishloq xo\'jaligi mahsulotlari sifatini baholash markazi" DM 2023 yil hosilidan jamg\'arilgan :name mahsulotlarini sertifikatlanishi bo\'yicha ma\'lumot', ['name' => $name]) }}
                        </li>
                    </ol>
                </div>
                <!-- filter component -->
                <div class="row pb-2">
                    <div class="col-sm-4">
                        <select class="w-100 form-control state_of_country custom-select" style="background-color: #c5ccd0"
                            name="crop" id="crop">

                            @if (!empty($crop_names))
                                @foreach ($crop_names as $state)
                                    <option value="{{ $state->id }}"
                                        @if ($crop && $crop == $state->id) selected="selected" @endif> {{ $state->name }} </option>
                                @endforeach
                            @endif
                    ...
                    <div class="col-sm-4">
                        ...
                        <select class="w-100 form-control state_of_country custom-select" style="background-color: #c5ccd0"
                            ...
                            @if (!empty($years))
                                <option value="">{{ trans('app.Ishlab chiqarilgan sana') }}</option>
                                @foreach ($years as $key => $name)
                                    <option value="{{ $key }}"
                                        @if ($year && $year == $key) selected="selected" @endif> {{ $name }} </option>
                                @endforeach
                            @endif
                                    ...
                                    <table class="table table-striped table-bordered " style="margin-top:20px;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th rowspan="2">
                                                    {{ trans('app.Ekishga talab etiladigan mahsulot miqdori', ['measure_type' => $measure_type]) }}
                                                </th>
                                                <th colspan="2">
                                                    {{ trans('app.Laboratoriya tahlillari uchun kelgan mahsulot miqdori', ['measure_type' => $measure_type]) }}
                                                </th>
                                                <th rowspan="2">
                                                    {{ trans('app.Sertifikatlashtirilgan mahsulot miqdori', ['measure_type' => $measure_type]) }}
                                                </th>
                                                <th rowspan="2">
                                                    {{ trans('app.Sertifikatlangan mahsulot miqdori', ['measure_type' => $measure_type]) }}
                                                </th>
                                                <th rowspan="2">%</th>
                                                <th rowspan="2">
                                                    {{ trans('app.Sifat bo\'yicha nomuvofiq deb topilgan mahsulot miqdori', ['measure_type' => $measure_type]) }}
                                                </th>
                                                <th rowspan="2">%</th>
                                                <th rowspan="2">
                                                    {{ trans('app.Tahlilda turgan mahsulot miqdori', ['measure_type' => $measure_type]) }}
                                                </th>
                                                <th rowspan="2">%</th>
                                            </tr>
                                            <tr>
                                        ...
                                        <tbody>
                                                    ...
                                                    <td style="width: 150px">
                                                        <div class="input-container">
                                                            <input type="number" step="0.01" class="form-control" name="amount"
                                                                id="amount{{ $app->state_id }}"
                                                                onchange="saveAnswer({{ $app->state_id }} , this)"
                                                                value="{{ $app->required_amount }}"
                                                                @if ($app->required_amount) {{ 'disabled' }} >
                                                         <i class="fa fa-pencil" onclick="changeDisplay(this,{{ $app->state_id }})"></i> @endif
                                                                </div>
                                                    </td>
                                                    <td>{{ round($app->total_amount ?? 0, 2) }}</td>
                                                    <td>{{ $app->required_amount ? round((100 * $app->total_amount) / $app->required_amount) : '100' }}%
                                                    </td>
                                                    <td>{{ round(($app->total_amount ?? 0) - $app->total_amount_type_3, 2) }}</td>
                                                    <td>{{ round($app->total_amount_type_2, 2) }}</td>
                                                    <td>{{ $app->required_amount ? round((100 * $app->total_amount_type_2) / $app->required_amount) : '100' }}%
                                                    </td>
                                                    <td>{{ round($app->total_amount_type_4, 2) }}</td>
                                                    <td>{{ $app->required_amount ? round((100 * $app->total_amount_type_4) / $app->required_amount) : '100' }}%
                                                    </td>
                                                    <td>{{ round($app->total_amount_type_3 ?? 0, 2) }}</td>
                                                    <td>{{ $app->required_amount ? round((100 * $app->total_amount_type_3) / $app->required_amount) : '100' }}%
                                                    </td>
                                            ...
                                            <tr class="table-responsive_total">
                                                ...
                                                <td>{{ $total_required ? round((100 * $total_amount) / $total_required) : '100' }}%
                                                </td>
                                                <td>{{ round(($total_amount ?? 0) - $total_amount3, 2) }}</td>
                                                <td>{{ round($total_amount2, 2) }}</td>
                                                <td>{{ $total_required ? round((100 * $total_amount2) / $total_required) : '100' }}%
                                                </td>
                                                <td>{{ round($total_amount4, 2) }}</td>
                                                <td>{{ $total_required ? round((100 * $total_amount4) / $total_required) : '100' }}%
                                                </td>
                                                <td>{{ round($total_amount3, 2) }}</td>
                                                <td>{{ $total_required ? round((100 * $total_amount3) / $total_required) : '100' }}%
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
            ...
            <div style="display: none">
                <div id="invoice-cheque" class="py-4 col-12">
                    <h2>{{ trans('message."Qishloq xo\'jaligi mahsulotlari sifatini baholash markazi" DM 2023 yil hosilidan jamg\'arilgan :name mahsulotlarini sertifikatlanishi bo\'yicha ma\'lumot', ['name' => $name]) }}
                    </h2>
                    <div class="card">
                        <div class="card-body">
                            <div class="table-responsive">
