...
@section('styles')
    <style>
        th {
            background-color: #42b6fd !important; /* gradient from orange to dark orange */
        ...
        <div class="section">
            ...
            <div class="row">
                <div class="col-sm-3 pt-2" >
                    <a class="btn btn-success" href="{{ route('excel.export',[
    'from'=>$from,
    'till'=>$till,
    'city'=>$city,
    'crop'=>$crop,
    ...
    'year'=>$year
                ...
                <div class="col-sm-9" id="list-date-filter" style="padding-top: 3px;">
                    <div class="show-date btn btn-default filter-button" style="background-color: #3160EE;color:white"><b>{{trans('message.Vaqt bo\'yicha filtrlash')}}</b> <i
                            class="fa {{ ($from && $till) ? 'fa-angle-left':'fa-angle-right' }}" style="margin-left: 5px"></i></div>
                            ...
                            <div class="table-responsive text-nowrap" >
                                <table class="table table-striped table-bordered" style="margin-top:20px;" >
                                    ...
                                    <tr style="background-color: #90aec6 !important;">
                                        ...
                                        <td></td>
                                        <td >
                                            <select class="w-100 form-control state_of_country custom-select " name="city" id="city"  url="{!! url('/getcityfromstate') !!}">
                                                @if(count($states))
                                                    <option value="">{{trans('app.Viloyat tanlang')}}</option>
                                                @endif
                                                @if(!empty($states))
                                                    @foreach($states as $state)
                                                        <option value="{{ $state->id }}" @if( ($city && $city == $state->id))  selected="selected" @endif> {{$state->name}} </option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </td>
                                        <td>
                                            <select class="form-control w-100 city_of_state custom-select" name="region" id="region">
                                                    <option value="">{{trans('app.Tumanni tanlang')}}</option>
                                                @if(!empty($cities))
                                                    @foreach($cities as $city)
                                                        <option value="{{ $city->id }}"
                                                                @if(($region && $region==$city->id) || count($cities)==1)
                                                                selected="selected"
                                                            @endif
                                                        > {{$city->name}} </option>
                                                    @endforeach
                                        ...
                                        <td>
                                            ...
                                            @if($organization)
                                             <i class="fa fa-trash" style="color:red" onclick="changeDisplay('organization')"></i>
                                            @endif
                                        </td>
                                        <td>
                                            ...
                                            @if($prepared)
                                                <i class="fa fa-trash" style="color:red" onclick="changeDisplay('prepared')"></i>
                                            @endif
                                        </td>
                                        <td>
                                            <select class="w-100 form-control" name="country" id="country">
                                                ...
                                                @if(!empty($countries))
                                                    @foreach($countries as $name)
                                                        <option value="{{ $name->id }}"  @if($name->id == $country ) selected @endif
                                                        > {{$name->name}} </option>
                                                    @endforeach
                                            ...
                                            <select class="w-100 form-control name_of_corn custom-select" name="name" id="crops_name"
                                                ...
                                                @if(!empty($names))
                                                    @foreach($names as $name)
                                                        <option value="{{ $name->id }}" @if($crop == $name->id) selected @endif
                                                        > {{$name->name}} </option>
                                                    @endforeach
                                            ...
                                            <select class="form-control w-100 type_of_corn custom-select" name="type" id="type">
                                                ...
                                                @if(!empty($types))
                                                    @foreach($types as $type_name)
                                                        <option value="{{ $type_name->id }}" @if($type == $type_name->id) selected @endif
                                    ...
                                    @foreach($apps as $app)
                                        <tr>
                                            ...
                                            <td>{{ optional(optional(optional($app->organization)->city)->region)->name }}</td>
