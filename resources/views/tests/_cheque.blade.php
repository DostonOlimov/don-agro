<div id="invoice-cheque" class="py-4 col-12 {{$classes ?? ''}}">
    ...
    <div>
        <h4>
            ...
            {{$decision->application->crops->pre_name}} urugʼlik &nbsp;{{$decision->application->crops->name->name}} &nbsp; mahsuloti &nbsp;
            {{$nds_type}}&nbsp;  {{$decision->application->crops->name->nds->number}}&nbsp;{{$decision->application->crops->name->nds->name}}
            talablariga muvofiq, sifat koʼrsatkichlarini sinovdan oʼtkazish quyidagi standartlardagi usullarni qoʼllagan xolda amalga oshirilsin.
        </h4>
    </div>
    <h4>2.Namunaning identifikatsiya raqami va vazni:</h4>
    @php $k=1;$count=$decision->count; @endphp
    <div>
        @php
